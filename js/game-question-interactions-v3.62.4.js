// Pahlawan Angka v3.62.4 — game-native KSSR/PBD/UASA response engine.
...
(()=>{
 ...
 function answerSpec(q,id,ctx={}){
  ...
  if(angleMode==='construct_error'){const target=text(q?.prompt).match(/membentuk\s+(\d+)°/i)||raw.match(/tanda\s+(\d+)°/i);if(target)return gated({kind:'angle_build',value:Number(target[1]),suffix:'°',min:0,max:180,step:5,diagnostic:'construction_correction'});}
  if(angleMode==='claim'&&/^(betul|salah)$/i.test(raw))return{kind:'judgement_gate',value:raw.toLowerCase()};
  const circleMode=id==='D6.CIRCLE'&&q?.competencyId==='6.2.2'?String(q?.archetypeId||'').replace(/^y6v2_6_2_2_/,''):'';
  if(circleMode==='steps'){const steps=raw.split(/\s*→\s*/).filter(Boolean);if(steps.length>1)return{kind:'step_sequence',steps,domain:'circle'};}
  if(circleMode==='construction_error'){const target=raw.match(/(\d+(?:\.\d+)?)\s*cm/i);if(target)return gated({kind:'circle_build',value:Number(target[1]),suffix:'cm',min:1,max:20,step:.5,diagnostic:'construction_correction'});}
  if(circleMode==='design_compare'){
   const p=text(q?.prompt),ma=p.match(/A[^;]*?(jejari|diameter)\s+(\d+(?:\.\d+)?)\s*cm/i),mb=p.match(/B[^.]*?(jejari|diameter)\s+(\d+(?:\.\d+)?)\s*cm/i);
   if(ma&&mb){const opening=m=>Number(m[2])/(m[1].toLowerCase()==='diameter'?2:1),correct=/^B\b/i.test(raw)?'B':'A';return{kind:'circle_compare',choices:[{id:'A',opening:opening(ma)},{id:'B',opening:opening(mb)}],correct};}
  }
  if(circleMode==='claim'){
   const verdict=raw.match(/^(betul|salah)/i)?.[1]?.toLowerCase(),correction=raw.match(/(\d+(?:\.\d+)?)\s*cm/i);
   if(verdict&&correction)return gated({kind:'circle_claim',verdict,value:Number(correction[1]),min:1,max:20,step:.5});
   if(verdict)return{kind:'judgement_gate',value:verdict};
  }
  const spaceMode=id==='D6.SPACE_PROBLEM'&&q?.competencyId==='6.3.1'?String(q?.archetypeId||'').replace(/^y6v2_6_3_1_/,''):'';
  if(spaceMode==='tool_choice'||spaceMode==='integrated_design'){
   const setup=raw.match(/jangka(?:\s+bukaan)?\s+(\d+(?:\.\d+)?)\s*cm\s+dan\s+protraktor\s+(\d+)°/i);
   if(setup)return gated({kind:'space_tool_setup',compass:Number(setup[1]),angle:Number(setup[2]),compassLabel:'Bukaan jangka',angleLabel:'Sudut protraktor',minCompass:1,maxCompass:20,stepCompass:.5,minAngle:0,maxAngle:180,stepAngle:5});
  }
  if(spaceMode==='multi_step'){
   const combo=raw.match(/^(\d+)°\s+dan\s+(\d+(?:\.\d+)?)\s*cm$/i);
   if(combo)return gated({kind:'space_tool_setup',compass:Number(combo[2]),angle:Number(combo[1]),compassLabel:'Bukaan jangka',angleLabel:'Jumlah sudut',minCompass:1,maxCompass:20,stepCompass:.5,minAngle:0,maxAngle:180,stepAngle:5});
  }
  if(spaceMode==='error_analysis'){
   const correction=raw.match(/(\d+(?:\.\d+)?)\s*cm/i);
   if(correction)return gated({kind:'circle_build',value:Number(correction[1]),suffix:'cm',min:1,max:20,step:.5,diagnostic:'space_error_correction'});
  }
  if(spaceMode==='compare_design'){
   const p=text(q?.prompt),ma=p.match(/A\s+(jejari|diameter)\s+(\d+(?:\.\d+)?)\s*cm/i),mb=p.match(/B\s+(jejari|diameter)\s+(\d+(?:\.\d+)?)\s*cm/i);
   if(ma&&mb){const opening=m=>Number(m[2])/(m[1].toLowerCase()==='diameter'?2:1);return{kind:'circle_compare',choices:[{id:'A',opening:opening(ma)},{id:'B',opening:opening(mb)}],correct:/^B\b/i.test(raw)?'B':'A',domain:'space'};}
  }
  ...
  if((m=raw.match(/^RM\s*(-?[\d,]+(?:\.\d{1,2})?)$/i)))return gated({kind:'rune_entry',value:number(m[1]),prefix:'RM',dp:(m[1].split('.')[1]||'').length});
  if((m=raw.match(/^(-?[\d,]+(?:\.\d+)?)\s*%$/)))return gated({kind:'rune_entry',value:number(m[1]),suffix:'%',dp:(m[1].split('.')[1]||'').length});
  if(id==='D6.ANGLE'&&(m=raw.match(/^(\d+)°$/)))return gated({kind:q?.competencyId==='6.1.2'?'angle_build':'rune_entry',value:Number(m[1]),suffix:'°',min:0,max:180,step:5});
  if(id==='D6.SPACE_PROBLEM'&&(m=raw.match(/^(\d+)°$/)))return gated({kind:'rune_entry',value:Number(m[1]),suffix:'°',dp:0});
  if((id==='D6.CIRCLE'||id==='D6.SPACE_PROBLEM')&&/6\.2\.2|circle_draw/.test(`${q?.competencyId||''} ${(q?.misconceptionTargets||[]).join(' ')}`)&&(m=raw.match(/^(\d+(?:\.\d+)?)\s*cm$/)))return gated({kind:'circle_build',value:Number(m[1]),suffix:'cm',min:1,max:20,step:.5});
  if((m=raw.match(/^(-?[\d,]+(?:\.\d+)?)\s*(mm|cm|m|km|g|kg|mL|L|cm²|m²|km²|cm³|m³)$/i)))return gated({kind:'rune_entry',value:number(m[1]),suffix:m[2],dp:(m[1].split('.')[1]||'').length});
  if((m=raw.match(/^(-?\d+)\s+(\d+)\/(\d+)$/)))return gated({kind:'fraction_build',whole:Number(m[1]),numerator:Number(m[2]),denominator:Number(m[3]),mixed:true});
  if((m=raw.match(/^(-?\d+)\/(\d+)$/)))return gated({kind:'fraction_build',whole:0,numerator:Number(m[1]),denominator:Number(m[2]),mixed:false});
  if((m=raw.match(/^\((\d+)\s*,\s*(\d+)\)$/)))return gated({kind:'coordinate_plot',x:Number(m[1]),y:Number(m[2])});
  if((id.includes('.TIME')||/masa|waktu|jam/i.test(q?.kind||''))&&(m=raw.match(/^(\d{1,2}):(\d{2})$/)))return gated({kind:'time_dial',hour:Number(m[1]),minute:Number(m[2])});
  const sequence=raw.split(/\s*,\s*/);if(sequence.length>=2&&sequence.every(x=>/^-?[\d.]+$/.test(x)))return gated({kind:'sequence_build',tokens:sequence});
  if(typeof q?.answer==='number'&&Number.isFinite(q.answer))return gated({kind:'rune_entry',value:q.answer,dp:0});
  if(/^-?[\d,]+(?:\.\d+)?$/.test(compact))return gated({kind:'rune_entry',value:number(compact),dp:(compact.split('.')[1]||'').length});
 ...
 function prepare(q,ctx={}){
  if(!q)return q;const id=ctx.skillId||q.skill||'',grade=ctx.meta?.grade||skillGrade(id)||1,spec=answerSpec(q,id,ctx);
  q.responseType='interactive';q.responseMode='game';q.interaction={type:spec.kind,spec};
  q.assessmentAlignment={...alignment(id,grade),grade};q.responseBattleTier=ctx?.battleTier||(ctx?.isBoss===true?'boss':'minion');q.gameInteractionVersion=VERSION;
  return q;
 }
 ...
 function wrongTag(q,value){
  ...
  if(type==='circle_build'){
   const got=number(text(value).replace(/cm/ig,'')),target=Number(spec.value);
   if(got!==null&&Number.isFinite(target)){
    if(numericEqual(got,target*2)||numericEqual(got*2,target))return'radius_diameter';
    if(Math.abs(got-target)<=1)return'circle_precision';
   }
   return'circle_draw';
  }
  if(type==='circle_compare')return q?.interaction?.spec?.domain==='space'?'space_compare':'circle_compare';
  if(type==='circle_claim')return'circle_reasoning';
  if(type==='space_tool_setup'){
   const parts=text(value).split('|'),compass=number(parts[0]),angle=number(parts[1]);
   const cOk=numericEqual(compass,Number(spec.compass)),aOk=numericEqual(angle,Number(spec.angle));
   if(!cOk&&aOk)return numericEqual(compass,Number(spec.compass)*2)||numericEqual(compass*2,Number(spec.compass))?'radius_diameter':'space_compass';
   if(cOk&&!aOk)return'angle_construct';
   return'space_multi_step';
  }
  if(type==='step_sequence')return q?.interaction?.spec?.domain==='circle'?'circle_procedure':'angle_procedure';
 ...
 function renderSpaceToolSetup(q,root,ctx,spec){
  const wrap=document.createElement('div');wrap.className='paSpaceToolSetup';let compass=Math.max(spec.minCompass,Math.min(spec.maxCompass,5)),angle=90;
  const compassBox=document.createElement('div');compassBox.className='paSpaceControl paSpaceCompass';const cTitle=document.createElement('strong');cTitle.textContent=spec.compassLabel||'Bukaan jangka';const cVisual=document.createElement('div');cVisual.className='paCircleVisual';const cRing=document.createElement('i'),cLabel=document.createElement('b');cVisual.append(cRing,cLabel);
  const cRange=document.createElement('input');cRange.type='range';cRange.min=String(spec.minCompass);cRange.max=String(spec.maxCompass);cRange.step=String(spec.stepCompass);cRange.value=String(compass);cRange.setAttribute('aria-label',spec.compassLabel||'Bukaan jangka');cRange.oninput=()=>{compass=Number(cRange.value);cRing.style.width=cRing.style.height=`${44+compass*4}px`;cLabel.textContent=compass+' cm'};cRange.oninput();compassBox.append(cTitle,cVisual,cRange);
  const angleBox=document.createElement('div');angleBox.className='paSpaceControl paSpaceAngle';const aTitle=document.createElement('strong');aTitle.textContent=spec.angleLabel||'Sudut';const aVisual=document.createElement('div');aVisual.className='paAngleVisual';aVisual.innerHTML='<i></i><i></i><b>90°</b>';const arm=aVisual.children[1],aLabel=aVisual.children[2];
  const aRange=document.createElement('input');aRange.type='range';aRange.min=String(spec.minAngle);aRange.max=String(spec.maxAngle);aRange.step=String(spec.stepAngle);aRange.value=String(angle);aRange.setAttribute('aria-label',spec.angleLabel||'Sudut');aRange.oninput=()=>{angle=Number(aRange.value);arm.style.transform=`rotate(${-Math.min(angle,180)}deg)`;aLabel.textContent=angle+'°'};aRange.oninput();angleBox.append(aTitle,aVisual,aRange);
  const submit=submitButton(q,'Kunci pelan ruang');submit.onclick=()=>deliver(q,submit,compass+'|'+angle,ctx,numericEqual(compass,spec.compass)&&numericEqual(angle,spec.angle));wrap.append(compassBox,angleBox,submit);root.appendChild(wrap);
 }
 function renderCircleCompare(q,root,ctx,spec){
  const wrap=document.createElement('div');wrap.className='paCircleCompare';wrap.setAttribute('aria-label','Banding bukaan jangka');
  spec.choices.forEach(c=>{const b=button('', 'paCircleCompareChoice');b.dataset.questionToken=String(q.token);b.dataset.v=c.id;const ring=document.createElement('i'),label=document.createElement('b'),small=document.createElement('small');ring.style.width=ring.style.height=`${48+c.opening*7}px`;label.textContent='Reka bentuk '+c.id;small.textContent='Bukaan jangka '+c.opening+' cm';b.append(ring,label,small);b.onclick=()=>deliver(q,b,c.id,ctx,c.id===spec.correct);wrap.appendChild(b)});root.appendChild(wrap);
 }
 function renderCircleClaim(q,root,ctx,spec){
  const wrap=document.createElement('div');wrap.className='paCircleClaim';let verdict=null,value=Math.max(spec.min,Math.min(spec.max,5));
  const gate=document.createElement('div');gate.className='paJudgementGate';['betul','salah'].forEach(v=>{const b=button(v==='betul'?'BETUL':'SALAH','paJudgementChoice '+v);b.onclick=()=>{verdict=v;gate.querySelectorAll('button').forEach(x=>x.classList.remove('selected'));b.classList.add('selected')};gate.appendChild(b)});
  const visual=document.createElement('div');visual.className='paCircleVisual';const ring=document.createElement('i'),label=document.createElement('b');visual.append(ring,label);
  const range=document.createElement('input');range.type='range';range.min=String(spec.min);range.max=String(spec.max);range.step=String(spec.step);range.value=String(value);range.setAttribute('aria-label','Bukaan jangka pembetulan');range.oninput=()=>{value=Number(range.value);ring.style.width=ring.style.height=`${44+value*4}px`;label.textContent=value+' cm'};range.oninput();
  const submit=submitButton(q,'Kunci penilaian');submit.onclick=()=>{if(!verdict){wrap.classList.add('needsValue');return}wrap.classList.remove('needsValue');deliver(q,submit,verdict+'|'+value+' cm',ctx,verdict===spec.verdict&&numericEqual(value,spec.value))};wrap.append(gate,visual,range,submit);root.appendChild(wrap);
 }
