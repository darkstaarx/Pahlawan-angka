// Pahlawan Angka v3.62.2 — game-native KSSR/PBD/UASA response engine.
...
(()=>{
 ...
 function answerSpec(q,id){
  ...
  if(circleMode==='construction_error'){const target=raw.match(/(\d+(?:\.\d+)?)\s*cm/i);if(target)return{kind:'circle_build',value:Number(target[1]),suffix:'cm',min:1,max:20,step:.5,diagnostic:'construction_correction'};}
  ...
  if(circleMode==='claim'){
   ...
   if(verdict&&correction)return{kind:'circle_claim',verdict,value:Number(correction[1]),min:1,max:20,step:.5};
