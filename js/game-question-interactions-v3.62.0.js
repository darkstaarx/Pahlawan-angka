// Pahlawan Angka v3.62.0 — game-native KSSR/PBD/UASA response engine.
...
(()=>{
 ...
 function wrongTag(q,value){
  ...
  return exact?.tag||q.wrong?.[0]?.tag||'generated';
 }
 function deliver(q,btn,value,ctx,correct){
  ...
  if(!['sigil_select','coordinate_plot'].includes(q.interaction?.type))setTimeout(()=>{if(sess?.q?.token===q.token){btn.disabled=false;btn.classList.remove('no')}},260);
 }
 ...
 function render(q,answers,ctx){
  ...
  const spec=q.interaction.spec,labels={rune_entry:'Rune nombor',fraction_build:'Tempa pecahan',time_dial:'Dail masa',coordinate_plot:'Peta koordinat',sequence_build:'Rantai urutan',angle_build:'Protraktor kuasa',circle_build:'Jangka bulatan',claim_reason:'Bina hujah',sigil_select:'Pilih sigil tepat'},root=rootFor(q,answers,labels[spec.kind]||'Cabaran interaktif');
  if(spec.kind==='rune_entry')renderRune(q,root,ctx,spec);else if(spec.kind==='fraction_build')renderFraction(q,root,ctx,spec);else if(spec.kind==='time_dial')renderTime(q,root,ctx,spec);else if(spec.kind==='coordinate_plot')renderCoordinate(q,root,ctx,spec);else if(spec.kind==='sequence_build')renderSequence(q,root,ctx,spec);else if(spec.kind==='angle_build')renderAngle(q,root,ctx,spec);else if(spec.kind==='circle_build')renderCircle(q,root,ctx,spec);else if(spec.kind==='claim_reason')renderClaimReason(q,root,ctx,spec);else renderSigils(q,root,ctx);return true;
 }
