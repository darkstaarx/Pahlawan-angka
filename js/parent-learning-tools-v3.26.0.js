...
function reportDiagnosis(snap){
 ...
 const cause=mis?(PARENT_MISCONCEPTION_COPY[mis[0]]||'cara yang sama menghasilkan kesalahan berulang'):hintRate>.4?'boleh menjawab dengan bantuan, tetapi langkahnya belum cukup kukuh untuk dibuat sendiri':lead.accuracy<60?'asas pengiraan belum konsisten apabila format soalan berubah':'kemahiran ini belum stabil walaupun jawapan betul sudah mula muncul';
 return{lead,strong,confidence,headline:`${snap.name} bukan lemah dalam semua Matematik. Jurang utamanya ialah ${lead.m.title}.`,cause:`Kemungkinan puncanya: ${cause}.`,evidence:`${lead.s.correct||0}/${lead.attempts} betul (${lead.accuracy}%) berbanding ${otherAccuracy}% pada kemahiran lain${gap?` — jurang ${gap} mata peratus`:''}.${Number(lead.s.hints||0)?` Petunjuk digunakan ${lead.s.hints} kali.`:''}`,action:REPORT_ACTIONS[mis?.[0]]||`Buat 8 hingga 10 soalan ${lead.m.title} dalam dua format berbeza, kemudian semak tanpa Petunjuk.`};
}
...
async function exportReport(){
 ...
 try{
  const snap=snapshot(),diagnosis=reportDiagnosis(snap),pages=[];let p=await page(`Mengapa ${snap.name} Tersekat?`,`Ringkasan Matematik Darjah ${snap.grade} | ${dateLabel()}`),ctx=p.ctx;
  ...
  p=await page('Apa Yang Perlu Dilihat',`${snap.name} | Bukti penting sahaja, bukan semua topik`);ctx=p.ctx;ctx.fillStyle='#17263a';ctx.font='900 30px Arial';ctx.fillText('Keutamaan sekarang',78,295);let y=335;const focus=[...snap.priority,...snap.developing].slice(0,3);if(focus.length)for(const x of focus)y=reportSkillRow(ctx,x,y,'focus');else{ctx.fillStyle='#687486';ctx.font='21px Arial';ctx.fillText('Belum ada jurang utama yang disahkan.',78,y);y+=70}
  ctx.fillStyle='#17263a';ctx.font='900 30px Arial';ctx.fillText('Yang boleh dijadikan sandaran',78,y+28);y+=65;const stable=snap.strong.slice(0,3);if(stable.length)for(const x of stable)y=reportSkillRow(ctx,x,y,'stable');else{ctx.fillStyle='#687486';ctx.font='21px Arial';drawWrapped(ctx,'Belum cukup bukti untuk menamakan kemahiran mantap. Ini tidak bermaksud anak lemah; sistem masih mengumpul jawapan dalam format berbeza.',78,y,1060,30,3);y+=105}
