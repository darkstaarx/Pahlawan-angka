...
function solutionFor(item){
 ...
 if(direct)return `${direct[1].replace(/\s+/g,' ')} = ${answer}`;
 ...
 if(/dibundarkan kepada\s+\d+/i.test(prompt)&&/manakah/i.test(prompt)){const target=(prompt.match(/dibundarkan kepada\s+(\d+)/i)||[])[1];return `${answer} berada dalam julat yang membundar kepada ${target}; semak digit penentu sebelum membundar.`}
 if(/bundarkan/i.test(prompt)&&numbers.length){const n=numbers[0],unit=/ribu/i.test(prompt)?1000:/ratus/i.test(prompt)?100:/puluh/i.test(prompt)?10:1,digit=Math.floor(n/(unit/10))%10,direction=digit>=5?'naik':'kekal';return `Digit penentu ialah ${digit}. Oleh sebab ${digit}${digit>=5?' ≥ 5':' < 5'}, bundarkan ${direction} kepada ${answer}.`}
 if(Number.isFinite(answerNumber)&&numbers.length>=2){
  ...
  if(almost(subtract,answerNumber))return `${numbers.join(' − ')} = ${answer}.`;
  if(almost(sum,answerNumber))return `${numbers.join(' + ')} = ${answer}.`;
  if(almost(divide,answerNumber))return `${numbers[0]} ÷ ${numbers[1]} = ${answer}.`;
  if(almost(product,answerNumber))return `${numbers.join(' × ')} = ${answer}.`;
 }
 if(/bentuk|sisi|bucu|permukaan|puncak/i.test(prompt))return `${item.hint} Ciri tersebut sepadan dengan ${answer}.`;
...
async function worksheet(includeAnswers){
 ...
 try{
  const pack=makeQuestions(),pages=[];for(let start=0;start<pack.items.length;start+=6){const subset=pack.items.slice(start,start+6),p=await page(pack.title,`${pack.snap.name} | Darjah ${pack.snap.grade} | ${pack.items.length} soalan`),ctx=p.ctx;let y=270;if(start===0){ctx.fillStyle='#526173';ctx.font='20px Arial';ctx.fillText('Nama: ______________________________',78,y);ctx.fillText(`Tarikh: ${dateLabel()}`,720,y);y+=62}for(let i=0;i<subset.length;i++){const item=subset[i],number=start+i+1;roundRect(ctx,78,y,1084,190,18,'#ffffff','#ddd8cc');ctx.fillStyle='#0e2b4a';ctx.font='900 22px Arial';ctx.fillText(`${number}.`,102,y+37);ctx.fillStyle='#17263a';ctx.font='23px Arial';const bottom=drawWrapped(ctx,item.prompt,147,y+37,970,30,3);if(item.choices?.length){ctx.font='18px Arial';item.choices.forEach((choice,index)=>{const col=index%2,row=Math.floor(index/2),x=147+col*470,cy=Math.max(y+98,bottom+12)+row*39;roundRect(ctx,x,cy,440,31,9,'#f4f6f8','#c9cfd4');ctx.fillStyle='#26364a';ctx.fillText('○  '+choice.value,x+12,cy+22)})}else{ctx.strokeStyle='#bfc5c9';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(147,Math.max(y+112,bottom+20));ctx.lineTo(1090,Math.max(y+112,bottom+20));ctx.stroke();ctx.fillStyle='#8993a0';ctx.font='16px Arial';ctx.fillText('Jawapan / ruang kerja',147,Math.max(y+140,bottom+48))}y+=208}footer(ctx,pages.length+1,'Pahlawan Angka | Worksheet murid');pages.push(p.canvas)}
  if(includeAnswers){for(let start=0;start<pack.items.length;start+=8){const subset=pack.items.slice(start,start+8),p=await page('Skema dan Cara Menjawab',`${pack.title} | Untuk ibu bapa / penjaga`),ctx=p.ctx;let y=272;for(let i=0;i<subset.length;i++){const item=subset[i],number=start+i+1;roundRect(ctx,78,y,1084,158,16,i%2?'#f4f1e9':'#ffffff','#ddd8cc');ctx.fillStyle='#0e2b4a';ctx.font='900 18px Arial';drawWrapped(ctx,`${number}. ${item.prompt}`,102,y+27,1010,22,3);ctx.fillStyle='#9a6b17';ctx.font='900 14px Arial';ctx.fillText('CARA',102,y+98);ctx.fillStyle='#536174';ctx.font='17px Arial';drawWrapped(ctx,solutionFor(item),164,y+98,680,21,2);roundRect(ctx,862,y+101,274,42,12,'#102b4a','#d4aa35');ctx.fillStyle='#f5cf52';ctx.font='900 17px Arial';ctx.textAlign='center';ctx.fillText(`Jawapan: ${item.answer}`,999,y+128);ctx.textAlign='left';y+=169}footer(ctx,pages.length+1,'Pahlawan Angka | Skema penjaga');pages.push(p.canvas)}}
