...
function plainPrompt(raw){const div=document.createElement('div');div.innerHTML=String(raw||'');return(div.textContent||'').replace(/\s+/g,' ').trim()}
function printableQuestion(q){const raw=String(q?.prompt||'');if(!raw||/<(?:svg|img|canvas|table)\b/i.test(raw)||/(moneyVisual|clockFace|barChart|pictureGraph|coordinateGrid|shapeVisual|dataChart)/i.test(raw))return null;const prompt=plainPrompt(raw);if(prompt.length<3||prompt.length>260)return null;return{prompt,answer:String(q.answer??'').replace(/\s+/g,' ').trim(),hint:plainPrompt(q.hint||'Semak langkah pengiraan dengan teliti.'),skill:q.skill}}
