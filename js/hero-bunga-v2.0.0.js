...
(function(){
  ...
  function ensureFinisherFx(){
    ...
    let blast=arena.querySelector('.bunga-finisher-blast');
    if(!blast){blast=document.createElement('div');blast.className='bunga-finisher-blast';blast.setAttribute('aria-hidden','true');arena.appendChild(blast)}
    return{aura:makeFx('bungaFinalChargeAura','bunga-final-charge-aura',layer,f.finalAura),bloom:makeFx('bungaFinalBloom','bunga-final-bloom',arena,f.finalBloom),ring:makeFx('bungaFinalRing','bunga-final-ring',arena,f.finalBloom),blast};
  }
  function runFinisher(){
    ...
    afterFinisher(TIMING.finisher.form,()=>{const p=enemyCentre(arena,enemy),size=Math.min(arena.getBoundingClientRect().width*.4,p.size*1.22),blastSize=Math.min(arena.getBoundingClientRect().width*.72,p.size*2.5);[parts.bloom,parts.ring].forEach(x=>{positionAt(x,p);x.style.setProperty('--bunga-final-size',size+'px')});positionAt(parts.blast,p);parts.blast.style.setProperty('--bunga-blast-size',blastSize+'px');parts.bloom.src=heroData().fx?.finalBloom||'';parts.ring.src=heroData().fx?.finalBloom||'';parts.bloom.classList.remove('form','impact');parts.ring.classList.remove('form','compress');parts.blast.className='bunga-finisher-blast charge';void parts.blast.offsetWidth;parts.bloom.classList.add('form');parts.ring.classList.add('form');playBungaSfx('circle')});
    ...
    afterFinisher(TIMING.finisher.impact,()=>{parts.ring.classList.remove('form','compress');parts.bloom.src=heroData().fx?.finalImpact||'';parts.bloom.classList.remove('form','compress');parts.blast.className='bunga-finisher-blast explode';void parts.bloom.offsetWidth;parts.bloom.classList.add('impact');playBungaSfx('final')});
  ...
  function resetFinisher(){clearFinisherTimers();document.getElementById('bungaFinalChargeAura')?.classList.remove('active');document.querySelector('.bunga-finisher-blast')?.classList.remove('charge','explode');['bungaFinalBloom','bungaFinalRing'].forEach(id=>{const b=document.getElementById(id);if(b){b.classList.remove('form','compress','impact');b.removeAttribute('src')}})}
