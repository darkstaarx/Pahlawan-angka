// Pahlawan Angka v3.62.3 — game-native KSSR/PBD/UASA response engine.
...
(()=>{
 ...
 function wrongTag(q,value){
  ...
  if(type==='circle_compare')return'circle_compare';
 ...
 function render(q,answers,ctx){
  ...
  const spec=q.interaction.spec,labels={rune_entry:'Rune nombor',fraction_build:'Tempa pecahan',time_dial:'Dail masa',coordinate_plot:'Peta koordinat',sequence_build:'Rantai urutan',angle_build:'Protraktor kuasa',circle_build:'Jangka bulatan',circle_compare:'Banding jangka',circle_claim:'Semak pembinaan',claim_reason:'Bina hujah',tool_select:'Rak alat',step_sequence:'Susun prosedur',judgement_gate:'Gerbang keputusan',sigil_select:'Pilih sigil tepat'},root=rootFor(q,answers,labels[spec.kind]||'Cabaran interaktif');
  if(spec.kind==='rune_entry')renderRune(q,root,ctx,spec);else if(spec.kind==='fraction_build')renderFraction(q,root,ctx,spec);else if(spec.kind==='time_dial')renderTime(q,root,ctx,spec);else if(spec.kind==='coordinate_plot')renderCoordinate(q,root,ctx,spec);else if(spec.kind==='sequence_build')renderSequence(q,root,ctx,spec);else if(spec.kind==='angle_build')renderAngle(q,root,ctx,spec);else if(spec.kind==='circle_build')renderCircle(q,root,ctx,spec);else if(spec.kind==='circle_compare')renderCircleCompare(q,root,ctx,spec);else if(spec.kind==='circle_claim')renderCircleClaim(q,root,ctx,spec);else if(spec.kind==='claim_reason')renderClaimReason(q,root,ctx,spec);else if(spec.kind==='tool_select')renderToolSelect(q,root,ctx,spec);else if(spec.kind==='step_sequence')renderStepSequence(q,root,ctx,spec);else if(spec.kind==='judgement_gate')renderJudgement(q,root,ctx,spec);else renderSigils(q,root,ctx);return true;
 }
