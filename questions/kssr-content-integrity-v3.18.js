// Pahlawan Angka v3.18.0 — curriculum/content integrity guard.
...
(function(){
  ...
  const recentModes=id=>((window.sess&&sess.questionHistory)||[]).filter(x=>x.skillId===id).slice(-6).map(x=>String(x.archetypeId||'').replace(/^integrity_/,''));
  ...
  const REQUIREMENTS={
    ...
    'D2.2.3':[['mul_fact','mul_groups'],['mul10']], 'D2.2.4':[['div_exact'],['div_remainder']],
    ...
    'D6.MONEY':[['budget_multistep']], 'D6.TIME':[['speed_time','speed_distance','speed_rate']], 'D6.AREA':[['area_rect'],['perimeter_rect'],['volume_cuboid']]
  };
  ...
  window.PAContentIntegrity={version:'3.18.0',requirements:REQUIREMENTS,requirementStatus,modeFromQuestion};
  ...
  console.info('[Pahlawan Angka] Content Integrity Guard v3.18.0 active');
})();
