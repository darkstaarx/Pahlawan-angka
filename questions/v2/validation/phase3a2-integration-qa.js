...
ok(rt.templates.length===162,'template total remains 162');const t7=rt.templates.filter(t=>t.topicId==='D3.T7');ok(t7.length===26&&t7.filter(t=>t.responseType==='mcq').length===24&&t7.filter(t=>t.responseType==='interactive').length===2,'Topic 7 remains 26/24/2');
const expected={'D3.T1':18,'D3.T2':13,'D3.T3':30,'D3.T4':15,'D3.T5':15,'D3.T6':27,'D3.T7':26,'D3.T8':9,'D3.T9':9};for(const k of Object.keys(expected))ok(rt.templates.filter(t=>t.topicId===k).length===expected[k],k+' template count unchanged');
...
console.log(JSON.stringify({status:'pass',checks,version:release,standards:50,totalTemplates:162,topic7:{templates:26,battle:24,interactive:2},nonT7Mode:'shadow-only',semanticHardening:true,realPaperAlignment:true,supabaseMigration:false,liveExpansion:false},null,2));
