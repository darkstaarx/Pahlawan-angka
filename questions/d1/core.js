...
window.PAQuestionBanks.d1 = function(id,s,shift){
 ...
 if(id==="D1.FRAC"){let den=pick([2,4]),num=R(1,den-1),ans=`${num}/${den}`,q=Q(`Rajah dibahagi kepada <b>${den}</b> bahagian sama besar. Pilih pecahan yang perlu dilorek.`,ans,[N(`${den}/${num}`,"fraction"),N(`${Math.max(1,num-1)}/${den}`,"fraction"),N(`${num}/${den+1}`,"fraction")],"Pengangka ialah bahagian yang dilorek; penyebut ialah jumlah bahagian sama besar.","Darjah 1",true,true);return attachFractionShade(q,num,den)}
