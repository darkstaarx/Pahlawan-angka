...
for(const file of ['js/engine/intervention.js','js/engine/frontier.js','js/engine/adaptive.js','js/parent.js']){
 check(index.includes(`${file}?v=3.57.1`),`${file} is not cache-busted for v3.57.1`);
}
check(index.includes('js/battle.js?v=3.57.3'),'battle.js is not cache-busted for v3.57.3');
check(index.includes('js/version.js?v=3.57.3'),'version loader is not cache-busted for v3.57.3');
check(index.includes('js/pwa.js?v=3.57.3'),'PWA loader is not cache-busted for v3.57.3');
check(read('js/version.js').includes("PA_APP_VERSION='3.57.3'"),'release version is not v3.57.3');
check(read('sw.js').includes('v3.57.3'),'service worker release comment is stale');
