...
const assetPath='assets/heroes/wira-chibi/frames/ice-combo-spritesheet-v1.webp';
...
assert.equal(uint24le(24)+1,1280,'sprite sheet width changed');
assert.equal(uint24le(27)+1,512,'sprite sheet height changed');
...
assert(motion.includes("brightness(1.03) saturate(1.18)"),'attack colour correction missing');
assert(sw.includes(assetPath),'sprite sheet missing from app shell');
assert(index.includes('js/combat-motion-v1.js?v=3.62.23'),'renderer cache-bust missing');
assert(index.includes('js/version.js?v=3.62.23')&&index.includes('js/pwa.js?v=3.62.23'),'release cache-bust missing');
assert(version.includes("PA_APP_VERSION='3.62.23'"),'release version missing');
