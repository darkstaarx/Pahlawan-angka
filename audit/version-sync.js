...
assert.equal(appVersion,expected);assert.equal(cache,expected);assert.equal(integrityVersion,'3.18.1');assert.equal(sensoryVersion,'3.19.0');assert.equal(manipVersion,'3.19.1');assert.equal(dailyVersion,'3.20.0');assert.equal(devVersion,expected);assert.equal(combatVersion,expected);
...
for(const retired of ['js/world-response-v3.21.0.js','css/world-response-v3.21.0.css','js/dev-experiments-v3.21.1.js','css/dev-experiments-v3.21.1.css','audit/world-response-v3.21.0.js','audit/world-response-runtime-v3.21.0.js'])assert(!fs.existsSync(retired),`Retired file still present: ${retired}`);
assert(new RegExp(`dev-experiments-v${expected.replace(/\./g,'\\.')}`).test(sw));assert(new RegExp(`combat-polish-v${expected.replace(/\./g,'\\.')}`).test(sw));
