...
assert.equal(c('APP_VERSION'),expected);assert.equal(c('TYPED_UI_VERSION'),expected);
assert.equal(c('FINISHER_HOTSPOT_VERSION'),'3.21.6');assert.equal(c('TARGET_ANCHOR_VERSION'),'3.21.5');assert.equal(c('BOSS_LAB_VERSION'),'3.21.4');assert.equal(c('COMBAT_POLISH_VERSION'),'3.21.3');assert.equal(c('DEV_EXPERIMENTS_VERSION'),'3.21.2');
