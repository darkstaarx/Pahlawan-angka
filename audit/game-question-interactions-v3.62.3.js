...
const source=fs.readFileSync('js/game-question-interactions-v3.62.3.js','utf8');
const attrs={};
const ctx={console,Math,document:{documentElement:{setAttribute(k,v){attrs[k]=v}}}};ctx.window=ctx;
vm.createContext(ctx);vm.runInContext(source,ctx,{filename:'game-question-interactions-v3.62.3.js'});
const api=ctx.PAGameQuestionInteractions;
assert.equal(api.version,'3.62.3');assert.equal(attrs['data-game-question-interactions'],'3.62.3');
...
assert(sw.includes("'./js/game-question-interactions-v3.62.3.js'")&&sw.includes("'./css/game-question-interactions-v3.62.3.css'"),'offline shell assets missing');
