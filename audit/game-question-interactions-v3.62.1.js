...
const source=fs.readFileSync('js/game-question-interactions-v3.62.1.js','utf8');
const attrs={};
const ctx={console,Math,document:{documentElement:{setAttribute(k,v){attrs[k]=v}}}};ctx.window=ctx;
vm.createContext(ctx);vm.runInContext(source,ctx,{filename:'game-question-interactions-v3.62.1.js'});
const api=ctx.PAGameQuestionInteractions;
assert.equal(api.version,'3.62.1');assert.equal(attrs['data-game-question-interactions'],'3.62.1');
...
const pwa=fs.readFileSync('js/pwa.js','utf8'),sw=fs.readFileSync('sw.js','utf8'),html=fs.readFileSync('index.html','utf8'),version=fs.readFileSync('js/version.js','utf8'),css=fs.readFileSync('css/game-question-interactions-v3.62.1.css','utf8'),typed=fs.readFileSync('js/dev-experiments-v3.21.2.js','utf8');
