...
check(!!release&&(+release[1]>3||(+release[1]===3&&(+release[2]>32||(+release[2]===32&&+release[3]>=0)))),'version is older than action-variety v3.32.0');
check((action.match(/id:'original'/g)||[]).length===1,'Wira original contact attack is not retained');
check((action.match(/id:'(?:dash|arc|pulse)'/g)||[]).length===3,'Wira does not have original plus three new contact attacks');
check((action.match(/id:'(?:addition|subtraction|division)'/g)||[]).length===3,'Bunga does not have three portal contact attacks');
