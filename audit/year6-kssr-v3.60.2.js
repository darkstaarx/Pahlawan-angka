// Regression audit for KSSR Year 6 cognitive-depth hardening v3.60.2
const fs=require('fs'),vm=require('vm'),assert=require('assert'),path=require('path');
const src=fs.readFileSync(path.join(__dirname,'..','questions','kssr-year6-curriculum-v3.60.2.js'),'utf8');
...
vm.createContext(ctx);vm.runInContext(src,ctx,{filename:'kssr-year6-curriculum-v3.60.2.js'});
...
for(const id of ids){stats[id]={};for(const [level,state] of Object.entries(states)){const arch=new Set(),dem=new Set();let bad=0;
 for(let i=0;i<1000;i++){const q=ctx.window.PAQuestionBanks.d6(id,state,false);assert(q,id+'/'+level+' null');assert.equal(q.source,'kssr-year6-v3.60.2',id+'/'+level+' source');assert.equal(q.wrong.length,3,id+'/'+level+' distractors');const opts=[q.answer,...q.wrong.map(x=>x.v)].map(sem);if(new Set(opts).size!==4)bad++;arch.add(q.archetypeId||'prior');dem.add(q.demand||'prior');const t=sem(q.prompt);
