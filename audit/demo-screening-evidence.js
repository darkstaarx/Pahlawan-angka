...
assert.deepStrictEqual(JSON.parse(JSON.stringify(c.PADemo.summary())),{total:10,first:9,correct:10,hints:1,corrected:1,correctedWithHint:1,independent:9});
c.PAParentTools={demoPrintableItems:()=>Array(5).fill({})};c.PADemo.finish();assert.equal(element('paDemoWorksheet').disabled,false);assert.match(element('paDemoWorksheet').textContent,/5 soalan terpilih/);assert.strictEqual(c.db,originalDb);assert.strictEqual(c.sess,originalSess);assert.equal(c.db.coins,99);
...
c.PAParentTools={demoPrintableItems:()=>[]};c.PADemo.start();assert.equal(c.PADemo.summary().total,0);answer(true,false,true);answer(true,true,false);answer(false,true,true);
assert.deepStrictEqual(JSON.parse(JSON.stringify(c.PADemo.summary())),{total:3,first:1,correct:2,hints:2,corrected:1,correctedWithHint:0,independent:0});c.PADemo.finish();assert.match(element('resultCoach').textContent,/1 soalan belum berjaya/);assert.equal(element('paDemoWorksheet').disabled,true);
