...
vm.runInThisContext(fs.readFileSync('js/game-question-interactions-v3.62.4.js','utf8'),{filename:'game-question-interactions.js'});
...
let count=0,visual=0,reason=0,app=0,arch={},interactionFamilies=new Set(),d1Reason={},d1Local=0,d1Sticker=0;
for(const [bank,list] of Object.entries(ids)){
 for(const id of list){
  ...
  for(let i=0;i<120;i++){
   ...
   if(q.demand==='reasoning'){reason++;if(id.startsWith('D1.'))d1Reason[id]=(d1Reason[id]||0)+1;}if(q.demand==='application')app++;if(id.startsWith('D1.')){if(/kantin|karipap|kuih|roti canai|rambutan|bas sekolah|buku latihan|pau/i.test(q.prompt))d1Local++;if(/pelekat/i.test(q.prompt))d1Sticker++;}
...
assert(PAKSSRDepth.contractStatus['D6.FRAC']==='depth-v3.22.1');
...
for(const id of ids.d1){
 for(let i=0;i<30;i++){
  ...
  const q=PAQuestionBanks.d1(id,{mastery:10,evidence:0,confidence:10},false);
  assert(q&&q.demand!=='reasoning',id+' foundation learner received reasoning too early');
 }
}

console.log(`PASS runtime samples=${count}, visual=${visual}, reasoning=${reason}, application=${app}, d1Local=${d1Local}`);
